...
$discharges = ['White Discharge','Clear Discharge','Spotting'];
/* yMatrix has a row for each discharge.
in each row there is a column for each cycle day.
any cell indicates the number of entries in DB for a particular discharge and a particular cycle day */
$yMatrix = array();

//loop to iterate over each discharge
for($i=0 ; $i<sizeof($discharges) ; $i++)
{
	$discharge = $discharges[$i];
	//loop for each cycle day
	for($j=0 ; $j<$_POST[cycle_length] ; $j++)
	{	//getting all rows for jth cycle day where ith discharge is present
		$cd = $j+1;
		$query = "SELECT * FROM $username where cycle_day='$cd' AND discharge LIKE '%$discharge%'";
...
echo <<<_END
	...
	//add more colors here if more discharges are added
	var colors = ['red','blue','green'];
	
	//normalizing the y values for each discharge between 0 and 1
	...
	var discharges = ['Light','Medium','Heavy','Crime Scene'];
