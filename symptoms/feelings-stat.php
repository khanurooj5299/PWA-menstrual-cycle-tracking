...
$feelings = ['Happy','Emotional','Stressed','Calm','Tired','Irritated','Mood Swings','Sad'];
/* yMatrix has a row for each feeling.
in each row there is a column for each cycle day.
any cell indicates the number of entries in DB for a particular feeling and a particular cycle day */
$yMatrix = array();

//loop to iterate over each feeling
for($i=0 ; $i<sizeof($feelings) ; $i++)
{
	$feeling = $feelings[$i];
	//loop for each cycle day
	for($j=0 ; $j<$_POST[cycle_length] ; $j++)
	{	//getting all rows for jth cycle day where ith feeling is present
		$cd = $j+1;
		$query = "SELECT * FROM $username where cycle_day='$cd' AND feelings LIKE '%$feeling%'";
...
echo <<<_END
	...
	//add more colors here if more feelings are added
	var colors = ['red','blue','green','black','white','yellow','orange'];
	
	//normalizing the y values for each feeling between 0 and 1
	...
	var feelings = ['Happy','Emotional','Stressed','Calm','Tired','Irritated','Mood Swings','Sad'];
