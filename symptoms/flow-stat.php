...
$flows = ['Light','Medium','Heavy','Crime Scene'];
/* yMatrix has a row for each flow.
in each row there is a column for each cycle day.
any cell indicates the number of entries in DB for a particular flow and a particular cycle day */
$yMatrix = array();

//loop to iterate over each flow
for($i=0 ; $i<sizeof($flows) ; $i++)
{
	$flow = $flows[$i];
	//loop for each cycle day
	for($j=0 ; $j<$_POST[cycle_length] ; $j++)
	{	//getting all rows for jth cycle day where ith flow is present
		$cd = $j+1;
		$query = "SELECT * FROM $username where cycle_day='$cd' AND flow LIKE '%$flow%'";
...
echo <<<_END
	...
	//add more colors here if more flows are added
	var colors = ['red','blue','green','cyan'];
	
	//normalizing the y values for each flow between 0 and 1
	...
	var flows = ['Light','Medium','Heavy','Crime Scene'];
